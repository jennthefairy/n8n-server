...
import { airtableFetch, airtableGetRecord, airtableCreate, sanitizeParam } from '../lib/airtable.js';
...
export function registerCallbacks(bot: Bot) {
  ...
  bot.callbackQuery('myorders', async (ctx) => {
    ...
    // Find orders linked to campaigns owned by this user
    const campaigns = await airtableFetch('CAMPAIGNS', {
  ...
  bot.callbackQuery(/^campaign:(.+)$/, async (ctx) => {
    ...
    // If campaign has an image, send as photo
    if (f.image_url) {
      try {
  ...
  bot.callbackQuery('back:menu', async (ctx) => {
    ...
    // Try to edit; if it was a photo message, delete and send new
    try {
